...
class GradSchool:
    def __init__(self, name, state, city, rank, is_tied, score, school_url, school_id, fice_code):
        self.__name = name
        self.__state = state
        self.__city = city
        ...
        try:
            self.__school_id = int(school_id)
        except (ValueError, TypeError) as e:
            self.__school_id = school_id

        try:
            self.__fice_code = int(fice_code)
        except (ValueError, TypeError) as e:
            self.__fice_code = fice_code


    @classmethod
    def getFromJSON(cls, json_data):
        name = state = city = rank = is_tied = score = url = id = fice_code = None
        ...
        try:
            fice_code = json_data['fice_code']
        except KeyError:
            pass

        return cls(name, state, city, rank, is_tied, score, url, id, fice_code)
    ...
    def __iter__(self):
        ...
        yield self.__school_id
        yield self.__fice_code

    def __str__(self):
        return "name : {} \nstate : {} \ncity : {} \nrank : {} \nis tied : {} \nscore : {} \nschool url : {} \nschool id : {} \nfice code : {}".format(self.__name, self.__state, self.__city, self.__rank, self.__is_tied, 
                                                                                                                                                       self.__score, self.__school_url, self.__school_id, self.__fice_code)
...
class USNewsScrapper:
    ...
    def __get_column_headers(self):
        output_headers = ["Rank", "Graduate School Name", "State", "City", "Score", "US News URL", "ID", "FICE Code"]
